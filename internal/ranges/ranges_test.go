...
func Test_ParseRange(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			rangeStr: "",
			want: RangeQuery{
				Parts: []RangeQueryPart{{OpenIndex: 0, CloseIndex: -1}},
			},
			wantErr: false,
		},
		{
			...
			rangeStr: "42",
			want: RangeQuery{
				Parts: []RangeQueryPart{{OpenIndex: 42, CloseIndex: 43}},
			},
			wantErr: false,
		},
		{
			...
			rangeStr: "5, 6, 7",
			want: RangeQuery{
				Parts: []RangeQueryPart{{OpenIndex: 5, CloseIndex: 8}},
			},
			wantErr: false,
		},
		{
		...
		},
		{
			...
			rangeStr: "42..",
			want: RangeQuery{
				Parts: []RangeQueryPart{{OpenIndex: 42, CloseIndex: -1}},
			},
			wantErr: false,
		},
		{
			...
			rangeStr: "..42",
			want: RangeQuery{
				Parts: []RangeQueryPart{{OpenIndex: 0, CloseIndex: 43}},
			},
			wantErr: false,
		},
		{
			...
			rangeStr: "3..6",
			want: RangeQuery{
				Parts: []RangeQueryPart{{OpenIndex: 3, CloseIndex: 7}},
			},
			wantErr: false,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := ParseRange(tt.rangeStr)
